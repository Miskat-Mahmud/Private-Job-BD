...
public class EditProfileActivity extends AppCompatActivity implements Validator.ValidationListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == Config.RC_PICK_IMAGES) {
            ...
            if (resultCode == Activity.RESULT_OK && data != null) {
                ...
                docPath = saveInCache(selectedResume);
                if (docPath != null) {
    ...
    private String saveInCache(Uri uri) {
        String fileName = FileUtils.getFileName(EditProfileActivity.this, uri);
        File cacheDir = FileUtils.getDocumentCacheDir(EditProfileActivity.this);
        File file = FileUtils.generateFileName(fileName, cacheDir);
        String destinationPath = null;
        if (file != null) {
            destinationPath = file.getAbsolutePath();
            FileUtils.saveFileFromUri(EditProfileActivity.this, uri, destinationPath);
        }
    ...
    private void openFilePicker() {
        Intent resumePickerIntent = new Intent(Intent.ACTION_OPEN_DOCUMENT);
        resumePickerIntent.setType("application/pdf");
        resumePickerIntent.addCategory(Intent.CATEGORY_OPENABLE);
        resumePickerIntent.addFlags(Intent.FLAG_GRANT_WRITE_URI_PERMISSION | Intent.FLAG_GRANT_READ_URI_PERMISSION);
        startActivityForResult(resumePickerIntent, FILE_PICKER_REQUEST_CODE);
    }
